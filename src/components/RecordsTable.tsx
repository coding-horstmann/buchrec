...
export function RecordsTable({ records, links, onlyExceptions = false, onDisposition, onManualLink }: RecordsTableProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((record) => {
      if (onlyExceptions && (record.disposition !== "active" || !state.open.has(record.id) || !isReconciliationRecord(record))) return false;
  ...
  }, [records, onlyExceptions, query, state.open]);
